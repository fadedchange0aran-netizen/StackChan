...
static std::string NormalizeSpeechBubbleText(std::string_view text)
{
    ...
    for (char ch : text) {
        const bool is_break = ch == '\r' || ch == '\n' || ch == '\t';
        const bool is_space = ch == ' ' || is_break;
        if (is_space) {
...
void DefaultSpeechBubble::setSpeech(std::string_view text)
{
    ...
    auto normalized_text = NormalizeSpeechBubbleText(text);
    ...
    lv_text_get_size(&text_size, normalized_text.c_str(), _text->getTextFont(), 0, 0, LV_COORD_MAX, LV_TEXT_FLAG_NONE);

    int bubble_width = min(text_size.x + _text_mx * 2, _bubble_max_width);
    bubble_width     = max(bubble_width, _bubble_min_width);
    int text_width   = bubble_width - _text_mx * 2;
    bool needs_scroll = text_size.x > text_width;
    ...
    _text->setLongMode(needs_scroll ? LV_LABEL_LONG_MODE_SCROLL_CIRCULAR : LV_LABEL_LONG_MODE_CLIP);
    _text->setText(normalized_text);

    setVisible(true);
    lv_obj_update_layout(_bubble->get());
    lv_obj_update_layout(_text->get());
    lv_obj_invalidate(_text->get());
    lv_obj_invalidate(_bubble->get());
    lv_obj_invalidate(_container->get());
}
