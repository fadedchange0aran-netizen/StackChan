...
static constexpr std::string_view _native_mode_nvs_ns = "stack_mode";
static constexpr std::string_view _native_mode_nvs_key = "prefer_native";

static bool _is_dummy_ota_url(std::string_view url)
{
    return url.find("127.0.0.1") != std::string_view::npos ||
           url.find("localhost") != std::string_view::npos ||
           url.find("dummy") != std::string_view::npos;
}
...
void Hal::startXiaozhi()
{
    ...
    // Keep the VPS bridge available even if the UI later returns to launcher.
    startVpsBridgeIfNeeded();

    hal_bridge::start_xiaozhi_app();
}

void Hal::startVpsBridgeIfNeeded()
{
    if (_vps_bridge_started) {
        return;
    }
    _vps_bridge_started = true;
    mclog::tagInfo(_tag, "start vps bridge");
    // Lower priority from 5 to 2 to avoid starving system initialization/activation
    xTaskCreate(vps_bridge_task, "vps_bridge", 16384, NULL, 2, NULL);
}

bool Hal::isNativeModePreferred()
{
    Settings mode_settings(_native_mode_nvs_ns.data(), false);
    if (mode_settings.GetBool(_native_mode_nvs_key.data(), false)) {
        return true;
    }

    // Backfill older installs that already stored the custom websocket URL.
    Settings ws_settings("websocket", false);
    Settings wifi_settings("wifi", false);
    const auto ws_url = ws_settings.GetString("url");
    const auto ota_url = wifi_settings.GetString("ota_url");
    return !ws_url.empty() && _is_dummy_ota_url(ota_url);
}

void Hal::setNativeModePreferred(bool preferred)
{
    Settings mode_settings(_native_mode_nvs_ns.data(), true);
    mode_settings.SetBool(_native_mode_nvs_key.data(), preferred);
}
