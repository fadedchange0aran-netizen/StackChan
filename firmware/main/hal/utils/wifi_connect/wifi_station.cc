...
void StackChanWifiStation::Start()
{
    ...
    // Persist station credentials across reboot so the post-setup reboot
    // does not drop the just-configured Wi-Fi connection.
...
void StackChanWifiStation::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == WIFI_EVENT_STA_START) {
        ...
        // If credentials are configured, keep trying to recover from runtime disconnects.
        if (!this_->ssid_.empty()) {
