...
esp_err_t Ota::CheckVersion() {
    ...
    std::string ws_token = ws_settings.GetString("token");

    const bool is_dummy_ota_url = url.find("127.0.0.1") != std::string::npos ||
                                  url.find("localhost") != std::string::npos ||
                                  url.find("dummy") != std::string::npos;
    const bool is_custom_bridge_mode = ws_token == PRIVATE_BRIDGE_TOKEN ||
                                       ws_url.find(PRIVATE_DOMAIN_HINT) != std::string::npos;

    // Our custom VPS mode should never hit the upstream OTA server, otherwise
    // the application gets stuck in a retry loop before entering normal flow.
    if (is_dummy_ota_url || is_custom_bridge_mode) {
        ESP_LOGI(TAG, "Custom mode detected, skipping OTA network check");
    ...
    cJSON_Delete(root);
    has_new_version_ = false; // 强制禁用新版本标记
    return ESP_OK;
}
