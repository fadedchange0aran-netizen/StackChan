...
ENV_FILE = Path(os.getenv("STACKCHAN_ENV_FILE", "/root/.aran_env")).expanduser()
...
CONFIG = {
    ...
    "GATEWAY_URL": env_str("STACKCHAN_GATEWAY_URL"),
    "GATEWAY_KEY": env_str("STACKCHAN_GATEWAY_KEY"),
...
def validate_config() -> None:
    ...
    if missing:
        missing_names = ", ".join(f"STACKCHAN_{key}" for key in missing)
        raise RuntimeError(
...
def resolve_robot_id(robot_id: str = "") -> tuple[str | None, str | None]:
    resolved = (robot_id or CONFIG["DEFAULT_ROBOT_ID"]).strip()
    if not resolved:
        return None, "错误：未提供 robot_id，且未配置 STACKCHAN_DEFAULT_ROBOT_ID。"
    return resolved, None


def validate_robot_online(robot_id: str) -> str | None:
    if robot_id not in connected_robots:
        return f"错误：机器人 {robot_id} 未在线。"
    return None
...
def is_echo_like_text(robot_id: str, text: str) -> bool:
    ...
    history = robot_histories.get(robot_id)
    if not history:
        return False
...
async def send_robot_message(robot_id: str, payload: dict) -> None:
    robot_socket = connected_robots[robot_id]
...
def build_gateway_messages(robot_id: str, user_text: str) -> list[dict]:
    messages: list[dict] = [{"role": "system", "content": BUILTIN_VOICE_STYLE_PROMPT}]
    prompt = robot_prompts.get(robot_id, "").strip()
    if prompt:
        messages.append({"role": "system", "content": prompt})
    messages.extend(list(robot_histories[robot_id]))
    messages.append({"role": "user", "content": user_text})
    return messages


async def query_gateway(robot_id: str, user_text: str) -> str:
    messages = build_gateway_messages(robot_id, user_text)
    headers = {
        "Authorization": f"Bearer {CONFIG['GATEWAY_KEY']}",
        "Content-Type": "application/json",
    }
    body = {"model": CONFIG["ROBOT_MODEL_NAME"], "messages": messages}

    async with httpx.AsyncClient(timeout=CONFIG["HTTP_TIMEOUT_SECONDS"]) as client:
        response = await client.post(CONFIG["GATEWAY_URL"], json=body, headers=headers)
        response.raise_for_status()
        data = response.json()
        ai_text = sanitize_voice_text(data["choices"][0]["message"]["content"])

    robot_histories[robot_id].append({"role": "user", "content": user_text})
    robot_histories[robot_id].append({"role": "assistant", "content": ai_text})
    return ai_text
...
async def finalize_realtime_speech(session: VoiceSession, silence_seconds: float | None = None) -> None:
    ...
    logger.info(
        "[Voice] finalize robot=%s frames=%d bytes=%d silence=%.2fs",
        session.robot_id,
...
async def process_voice_request(session: VoiceSession) -> None:
    ...
    try:
        ...
        try:
            ...
            ai_text = await query_gateway(session.robot_id, user_text)
            gateway_elapsed_ms = int((time.perf_counter() - gateway_started_at) * 1000)
            logger.info("[Voice] %s gateway=%d ms", session.robot_id, gateway_elapsed_ms)
        except Exception as exc:
...
async def mcp__stackchan_switch_mode(
    ...
    await send_robot_json(connected_robots[robot_id], {"type": "set_mode", "xiaozhi": xiaozhi_mode})
...
async def mcp__stackchan_status(
    ...
    status = {
        "robot_id": robot_id,
        "control_channel_online": robot_id in connected_robots,
        "voice_session_online": session is not None,
        ...
        "has_robot_prompt": robot_id in robot_prompts and bool(robot_prompts[robot_id].strip()),
...
async def mcp__stackchan_clear_prompt(
    ...
    if error:
        return error
    if robot_id in robot_prompts:
        del robot_prompts[robot_id]
...
async def mcp__stackchan_reset_dialogue(
    ...
    if error:
        return error
    if robot_id in robot_histories:
        robot_histories[robot_id].clear()
...
async def mcp__stackchan_switch_official(
    ...
    await send_robot_json(connected_robots[robot_id], {"type": "set_mode", "xiaozhi": True})
...
async def mcp__stackchan_switch_native(
    ...
    await send_robot_json(connected_robots[robot_id], {"type": "set_mode", "xiaozhi": False})
...
async def main_voice_ws_handler(websocket: FastAPIWebSocket):
    ...
    robot_id = websocket.headers.get("device-id") or websocket.query_params.get("device_id") or "unknown-device"
        ...
        while True:
            ...
            if message.get("bytes") is not None:
                ...
                if raw_audio and session.listening:
                    if time.perf_counter() < session.listening_guard_until:
